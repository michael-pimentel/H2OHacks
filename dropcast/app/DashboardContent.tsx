  ...
  return (
    <div className="space-y-8">
      ...
      <div className="rounded-xl bg-gradient-to-r from-sky-900/30 to-slate-900 border border-blue-900/60 px-6 py-4 flex flex-wrap gap-6 items-center">
        <div>
          <div className="text-xs text-slate-400 uppercase tracking-wider mb-1">Total Water Stored</div>
          <div className="text-3xl font-bold text-white">
            {(totalAF / 1_000_000).toFixed(2)}M{" "}
            <span className="text-base font-normal text-slate-400">acre-feet</span>
          </div>
          <div className="text-xs text-slate-400">
            {systemPct}% full across 5 reservoirs · enough for ~{homesEquivalent}M homes for a year
          </div>
        </div>
        <div className="h-8 w-px bg-blue-900/60 hidden sm:block" />
        <div>
          <div className="text-xs text-slate-400 uppercase tracking-wider mb-1">Overall Health</div>
          ...
          <div className="text-xs text-slate-500 mt-1">
            {systemPct >= 60 ? "Reservoirs well supplied" : systemPct >= 45 ? "Slightly below normal" : systemPct >= 30 ? "Conservation advised" : "Critically low — restrictions likely"}
          </div>
        </div>
        <div className="h-8 w-px bg-blue-900/60 hidden sm:block" />
        <div>
